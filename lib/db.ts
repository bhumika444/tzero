...
export function initializeDatabase() {
  ...
  // Seed initial marketplace orders if empty
  seedTradingData()
}

function seedTradingData() {
  const systemUserId = 'user_system_mm'
  const systemEmail = 'marketmaker@tzero.com'

  const systemOrderCount = db.prepare('SELECT COUNT(*) as count FROM trading_orders WHERE user_id = ?').get(systemUserId) as { count: number }
  if (systemOrderCount.count > 0) return
  ...
  if (!existingSystem) {
    db.prepare(`
      ...
      VALUES (?, ?, ?, 'Market', 'Maker', datetime('now'))
    `).run(systemUserId, systemEmail, 'hashed_password_mm')
  }

  // Assets to seed (from secondaryTradingAssets.json)
  const assets = [
    { symbol: 'NVMT', price: 3.01 },
    { symbol: 'BIOT', price: 12.45 },
    { symbol: 'TECH', price: 42.15 },
    { symbol: 'HYPE', price: 0.85 },
    { symbol: 'ENRG', price: 18.20 }
  ]

  const insertOrder = db.prepare(`
    INSERT INTO trading_orders (id, user_id, symbol, side, quantity, remaining_quantity, price, status, time_in_force, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
  `)

  // Create some sell orders (Asks) for each asset
  assets.forEach(asset => {
    // Large liquidity providers
    insertOrder.run(`ord_mm_s1_${asset.symbol}`, systemUserId, asset.symbol, 'sell', 1000, 1000, asset.price, 'New', 'gtc')
    insertOrder.run(`ord_mm_s2_${asset.symbol}`, systemUserId, asset.symbol, 'sell', 500, 500, asset.price + 0.05, 'New', 'gtc')

    // Some buy orders (Bids) to populate the order book
    insertOrder.run(`ord_mm_b1_${asset.symbol}`, systemUserId, asset.symbol, 'buy', 800, 800, asset.price - 0.05, 'New', 'gtc')
  })
